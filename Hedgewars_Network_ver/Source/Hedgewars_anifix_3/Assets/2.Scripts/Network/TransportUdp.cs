...
public class TransportUDP : ITransport
{
	...
	// 같은 단말에서 실행할 때 판별용으로 리스닝 소켓의 포트 번호를 보존.
	...
	public void Dispatch()
	{
		...
		// 킵얼라이브.
		if (m_socket != null) {
			// 통신 상대에게 접속을 시작했음을 정기적으로 알린다.
			TimeSpan ts = DateTime.Now - m_keepAliveTicker;
			
			if (ts.Seconds > m_keepAliveInter || m_isFirst) {
				// UDP로 연결에서는 핸드쉐이크를 하지 않으므로.
				// 같은 단말에서 실행할 땐 포트 번호로 송신원을 판별해야만 한다.
				// 이 때문에 접속 트리거가 되는 킵얼라이브 패킷에 IP 주소와 포트 번호를.
				// 실어서 판별하게 했다.
