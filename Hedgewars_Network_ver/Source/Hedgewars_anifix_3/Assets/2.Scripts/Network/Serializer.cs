...
public class Serializer
{
	...
	public Serializer()
	{
        ...
        // 엔디언을 판정.
        // int 1을 바이트 변환 했을 때 맨 앞 바이트가 1이면 리틀엔디언이다.
        int val = 1;
	...
	{
		// byte열은 데이터 덩어리로서 설정하므로 엔디언 변환을 하지 않기 때문에
		// 버퍼를 저장할 곳에서 원래로 돌아가게 한다.
	...
	protected bool Deserialize(ref byte[] element, int length)
	{
		...
		// byte열은 데이터의 덩어리로서 보존되므로 엔디언 변환하지 않게
		// 버퍼를 여기서 원래로 되돌린다.
		if (m_endianness == Endianness.LittleEndian) {
			Array.Reverse(element);	
		}
